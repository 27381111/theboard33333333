...
// --- Chatboard (pulizia messaggi > 12h, presenza username univoci) ---
const CHAT_PRUNE_HOURS = 12;
const CHAT_PRESENCE_MINUTES = 5;

function _chatPresenceCutoff() {
  return new Date(Date.now() - CHAT_PRESENCE_MINUTES * 60 * 1000).toISOString();
}

async function pruneChatPresence() {
  await ensurePgInit();
  const cutoff = _chatPresenceCutoff();
  if (usePostgres && neonSql) {
    await neonSql`DELETE FROM chat_active_users WHERE last_seen_at < ${cutoff}`;
    return;
  }
  const db = getSqlite();
  db.prepare('DELETE FROM chat_active_users WHERE last_seen_at < ?').run(cutoff);
}

async function isChatUsernameTaken(username) {
  await ensurePgInit();
  const lower = (username || '').trim().toLowerCase() || 'anon';
  pruneChatPresence();
  if (usePostgres && neonSql) {
    const rows = await neonSql`SELECT 1 FROM chat_active_users WHERE username_lower = ${lower} LIMIT 1`;
    return rows.length > 0;
  }
  const db = getSqlite();
  return !!db.prepare('SELECT 1 FROM chat_active_users WHERE username_lower = ? LIMIT 1').get(lower);
}

async function touchChatUser(username) {
  await ensurePgInit();
  const name = (username || '').trim() || 'anon';
  ...
  if (usePostgres && neonSql) {
    await neonSql`
      INSERT INTO chat_active_users (username_lower, last_seen_at) VALUES (${lower}, ${now})
      ON CONFLICT (username_lower) DO UPDATE SET last_seen_at = ${now}
    `;
    return;
  }
  const db = getSqlite();
  db.prepare('INSERT OR REPLACE INTO chat_active_users (username_lower, last_seen_at) VALUES (?, ?)').run(lower, now);
}

async function getChatActiveCount() {
  await pruneChatPresence();
  if (usePostgres && neonSql) {
    const rows = await neonSql`SELECT COUNT(*) AS n FROM chat_active_users`;
    return (rows[0] && rows[0].n) ? Number(rows[0].n) : 0;
  }
  const db = getSqlite();
  const row = db.prepare('SELECT COUNT(*) AS n FROM chat_active_users').get();
  return row ? row.n : 0;
}
...
module.exports = { init, createUser, getUserByUsername, getUserById, getPosts, getPost, createPost, createPostAnon, incrementLikes, deletePost, setPinnedPost, getComments, createComment, getChatMessages, getLastChatMessageByUsername, addChatMessage, deleteChatMessage, isChatUsernameTaken, touchChatUser, pruneChatPresence, getChatActiveCount, usePostgres };
